colors = {
    ...
    blue = {
        color = vector:new(0.95, 0.95, 0.95),
        glowcolor = vector:new(0.15, 0.35, 0.85)
    },
    white = {
    ...
    orange = {
        color = vector:new(0.95, 0.95, 0.95),
        glowcolor = vector:new(1, 0.6, 0.4)
    }
}
...
function entity:setorange()
    self.color = colors.orange.color
    self.glowcolor = colors.orange.glowcolor
    self.glowalpha = 0.3
end
...
local splashqueue = {}
local splashnum = 0
local function splashinternal(data)
    if (doingsplash) then
        return
    end
    ...
    splashtext.label = data.text

    if (data.yoffset == nil) then
        splashtext.y = 180
    else
        splashtext.y = 180 + data.yoffset
    end

    if (type(data.value) == "string") then
        splashtext:settext(data.value)
    elseif (data.value ~= nil) then
        splashtext:setvalue(data.value)
    end

    splashtext.fontscale = 5
    splashtext:changefontscaleovertime(0.1)
    splashtext.fontscale = 2
    splashtext.alpha = 1

    if (not data.color or data.color == "yellow") then
    ...
    elseif (data.color == "orange") then
        splashtext:setorange()
    end

    if (data.sound) then
        player:playlocalsound(data.sound)
    end

    game:ontimeout(function()
        splashtext:fadeovertime(0.1)
        splashtext.alpha = 0
        splashtext:changefontscaleovertime(0.1)
        splashtext.fontscale = 5
        game:ontimeout(function()
...
function addsplash(data, color, value)
    if (type(data) == "table") then
        table.insert(splashqueue, data)
    else
...
function pingescapewarning()
    if (not escapewarningsplash) then
        ...
        escapewarningsplash.font = "objective"
...
function entity:spawnai(forcespawn, callback)
...
function isweapon(ent)
    return game:issubstr(ent.classname, "weapon_") == 1
end
...
function array:totable()
    ...
    for i = 1, #keys do
        if (type(keys[i]) == "number") then
            t[keys[i] + 1] = self[keys[i] + 1]
        end

        if (type(keys[i]) == "string") then
            t[keys[i]] = self[keys[i]]
        end
    end
